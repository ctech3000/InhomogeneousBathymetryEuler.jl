...
function compute_eta(phi_old::Vector{Float64},phi_curr::Vector{Float64},domain::DampedDomainProperties,Dt::Real,nχ::Int)
    phi_surface_curr = phi_curr[1:nχ+1]
    phi_surface_old = phi_old[1:nχ+1]
    μ_D_disc = domain.μ_D.(trans.x.(χs))

    eta_curr = -1/g*((phi_surface_curr.-phi_surface_old)/Dt .+ μ_D_disc.*phi_surface_curr)

    return eta_curr
end

function compute_eta(phi_old::Vector{Float64},phi_curr::Vector{Float64},domain::DomainProperties,Dt::Real,nχ::Int)
    phi_surface_curr = phi_curr[1:nχ+1]
    phi_surface_old = phi_old[1:nχ+1]

    eta_curr = -1/g*((phi_surface_curr.-phi_surface_old)/Dt)
...
function solve_all_timesteps(K::SparseMatrixCSC, K_init::SparseMatrixCSC, domain::AbstractDomain, trans::σTransform, χs::Vector{Float64}, σs::Vector{Float64}, time_vec::Vector{Float64}, facetvalues::FacetValues, dh::DofHandler, ch::ConstraintHandler, D_inflow_boundary::Vector{Vector{Float64}}; save_phi::Bool=false)
    ...
    for (t_idx,t_p) in ProgressBar(enumerate(time_vec))
        ...
        phi_curr = eval_bathuate_at_grid_nodes(dh,K\f,:phi)
        eta_curr = compute_eta(phi_old,phi_curr,domain,Dt,nχ)
