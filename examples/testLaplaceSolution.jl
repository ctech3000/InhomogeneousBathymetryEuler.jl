#some tests on whether the program is able to reproduce the analytic solution for given dirichlet boundary data and a flat bathymetry
# result: yes
...
phi_surface_curr = transformedAnalyticPotential(χs,0*χs,t_p,2.0,wave,trans)
...
# test wheter Neumann = 0 is fulfilled on floor
...
dn_ana_inflow = analyticPotential_dx(xs, zs, t_p, 2.0, wave)
...
for i = 1:size(phi_ana,1)
    for j = 1:size(phi_ana,2)
        χ = χs[i]
        σ = σs[j]
        val = transformedAnalyticPotential(χ,σ,t_p,2.0,wave,trans)
        phi_ana[i,j] = val
    end
end
f3 = Figure()
...
diff_full = abs.(phi_mat - phi_ana)
f4 = Figure()
ax4 = Axis3(f4[1, 1], xlabel = "χ", ylabel = "σ")
surface!(ax4,χs,σs,diff_full)
f4
