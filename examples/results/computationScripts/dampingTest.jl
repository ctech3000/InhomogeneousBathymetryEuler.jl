...
for idx_e = eachindex(extensions)
    ...
    all_etas, all_phis = solve_all_timesteps(LHS_matrix, LHS_matrix_init, K_init, M_T0, M_T1, domain, trans, χs, σs, time_vec, timeMethod, facetvalues, dh, ch, outflow, D_inflow_boundary,energy=energy, save_phi=(34,34,2));
    etas[idx_e] = all_etas
    energies[idx_e] = energy
end

time_vec = collect(LinRange(0,T,nt))
jldsave("examples/results/plottingScripts/dampingTestData.jld2";etas,energies,Dχs,Dσs,Dts,domains,wave,transs,time_vec,χss)