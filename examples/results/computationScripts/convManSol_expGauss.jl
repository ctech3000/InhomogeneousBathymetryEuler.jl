#computes data for the convergence computation with manufactured solution with the Gaussian-like bathymetry
using InhomogeneousBathymetryEuler
using Ferrite, JLD2

nχ_base = 10
factors = [2^i for i = 0:6]
bathTypes = ["gauss0.25","gauss0.5","gauss0.75"]
...
for idx_f = 1:nHeats1
    for idx_b = 1:nHeats2
        ...
        if bathTypes[idx_b] == "flat"
            bath = Bathymetry(bathPoints,-1.5*ones(Float64,nχ+1))
        elseif bathTypes[idx_b] == "gauss0.25"
            bath = Bathymetry(bathPoints,"Gauss",shift=0.75,bHeight=0.375,depth=-1.5)
        elseif bathTypes[idx_b] == "gauss0.5"
            bath = Bathymetry(bathPoints,"Gauss",shift=0.75,bHeight=0.75,depth=-1.5)
        elseif bathTypes[idx_b] == "gauss0.75"
            bath = Bathymetry(bathPoints,"Gauss",shift=0.75,bHeight=1.125,depth=-1.5)
        end
...
jldsave("examples/results/plottingScripts/convManSolData_expGauss.jld2"; errorsL2, errorsMax, Dχs, max_u_0s)