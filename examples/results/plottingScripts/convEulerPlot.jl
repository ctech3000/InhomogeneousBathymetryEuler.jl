...
data_b = load("examples/results/plottingScripts/temp_new/convEuler_BathyData.jld2")
phis_b = data_b["phis"]
nFacs = length(phis_b)
etas_b = data_b["etas"]
Dχs_b = data_b["Dχs"]
...
data_nb = load("examples/results/plottingScripts/temp_new/convEuler_noBathyData.jld2")
phis_nb = data_nb["phis"]
etas_nb = data_nb["etas"]
Dχs_nb = data_nb["Dχs"]
Dts_nb = data_nb["Dts"]
χs_nb = data_nb["χs1"]
time_vec_nb = data_nb["time_vec"]
...
ax12 = Axis(fig1[1,2],xlabel="Δχ",ylabel="error",xscale=log10,yscale=log10,title="max error")
for idx_b = 1:nBaths
...
avgMax = sum.([eocsL2_phi[idx_b] for idx_b=1:nBaths])/(nFacs-2)
print("average:\nL2: $(avgL2),  Max: $(avgMax)\n")
...
ax22 = Axis(fig2[1,2],xlabel="Δχ",ylabel="error",xscale=log10,yscale=log10,title="max error")
for idx_b = 1:nBaths
...
avgMax = sum.([eocsL2_eta[idx_b] for idx_b=1:nBaths])/(nFacs-2)
print("average:\nL2: $(avgL2),  Max: $(avgMax)\n")