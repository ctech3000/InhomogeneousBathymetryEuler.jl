...
errorsL2_t_b = [computeError([phis_b[idx_f][idx_t] for idx_f = 1:nFacs],pairs,Dχs_b,norm="L2") for idx_t = eachindex(time_vec)]
errorsL2_b = [maximum([errorsL2_t_b[idx_t][idx_f] for idx_t = eachindex(time_vec)]) for idx_f = 1:nPairs]
errorsMax_t_b = [computeError([phis_b[idx_f][idx_t] for idx_f = 1:nFacs],pairs,Dχs_b,norm="max") for idx_t = eachindex(time_vec)]
...
Dχs_nb = data_nb["Dχs"]
Dts_nb = data_nb["Dts"]
errorsL2_t_nb = [computeError([phis_nb[idx_f][idx_t] for idx_f = 1:nFacs],pairs,Dχs_nb,norm="L2") for idx_t = eachindex(time_vec)]
errorsL2_nb = [maximum([errorsL2_t_nb[idx_t][idx_f] for idx_t = eachindex(time_vec)]) for idx_f = 1:nPairs]
errorsMax_t_nb = [computeError([phis_nb[idx_f][idx_t] for idx_f = 1:nFacs],pairs,Dχs_nb,norm="max") for idx_t = eachindex(time_vec)]
...
errorsL2_t_b = [computeError([etas_b[idx_f][idx_t] for idx_f = 1:nFacs],pairs,Dχs_b,norm="L2") for idx_t = eachindex(time_vec)]
errorsL2_b = [maximum([errorsL2_t_b[idx_t][idx_f] for idx_t = eachindex(time_vec)]) for idx_f = 1:nPairs]
errorsMax_t_b = [computeError([etas_b[idx_f][idx_t] for idx_f = 1:nFacs],pairs,Dχs_b,norm="max") for idx_t = eachindex(time_vec)]
errorsMax_b = [maximum([errorsMax_t_b[idx_t][idx_f] for idx_t = eachindex(time_vec)]) for idx_f = 1:nPairs]

errorsL2_t_nb = [computeError([etas_nb[idx_f][idx_t] for idx_f = 1:nFacs],pairs,Dχs_nb,norm="L2") for idx_t = eachindex(time_vec)]
errorsL2_nb = [maximum([errorsL2_t_nb[idx_t][idx_f] for idx_t = eachindex(time_vec)]) for idx_f = 1:nPairs]
errorsMax_t_nb = [computeError([etas_nb[idx_f][idx_t] for idx_f = 1:nFacs],pairs,Dχs_nb,norm="max") for idx_t = eachindex(time_vec)]
errorsMax_nb = [maximum([errorsMax_t_nb[idx_t][idx_f] for idx_t = eachindex(time_vec)]) for idx_f = 1:nPairs]
...
ax21 = Axis(fig2[1,1],xlabel="Δχ",ylabel="error",xscale=log2,yscale=log2,title="L2 error")
for idx_b = 1:nBaths
    lines!(ax21,Dχs_b[pairsFirst],errorsL2[idx_b],label=baths[idx_b])
end
axislegend(ax21,position=:rb)
ax22 = Axis(fig2[1,2],xlabel="Δχ",ylabel="error",xscale=log2,yscale=log2,title="max error")
for idx_b = 1:nBaths
